...
void saveCovariance(const std::vector<std::string>& xmlFiles, const std::string& outFileName, double epsilon, unsigned cauchyNumber){
  ...
  for(const auto& xmlStream : xmlStreams) unstableStates.emplace_back(treeParser.read(*xmlStream));//parse the xml files
  ...
  if(unstableStates.size() == 1){
    ...
    std::cout<<correlationEstimator<<std::endl;
    ...
    mean1.setErrorsFrom(correlationEstimator.getCovarianceMatrix());
    ...
    mean2.setErrorsFrom(correlationEstimator.getCovarianceMatrix());
...
int main (int argc, char* argv[]){
  ...
  if(arguments.count("help")) std::cout<<optionDescription<<std::endl;
	...
	if(regularFiles) saveCovariance(xmlFiles, arguments["output"].as<std::string>(), arguments["accuracy"].as<double>(), arguments["consecutive"].as<unsigned>());
