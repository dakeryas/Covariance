...
class TreeParser{ //class to parse xml files containing decay trees and build an unstable state from the root state of the xml file
  ...
  void parse(const boost::property_tree::ptree& tree, State<T>& state, bool normaliseData);//parse the children of tree and append them to state
  
public:
  TreeParser()= default;
  std::unique_ptr<UnstableState<T>> read(std::istream& stream, bool normaliseData);//read the tree defined in "stream" and normalise the data found if demanded
  
};

template <class T>
void TreeParser<T>::parse(const boost::property_tree::ptree& tree, State<T>& state, bool normaliseData){
  ...
  for(const auto& node : tree){
    ...
    if(node.first == "US"){
      ...
      parse(node.second, *(state.getDaughters().back()), normaliseData);//if the state is unstable, it means we have to parse deeper into the decay tree add append to the newly added unstable state
...
template <class T>
std::unique_ptr<UnstableState<T>> TreeParser<T>::read(std::istream& stream, bool normaliseData){
