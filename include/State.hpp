...
std::ostream& operator<<(std::ostream& output, const State<T>& state){
  ...
  for(auto it = state.getDaughters().begin(); it != state.getDaughters().end(); ++it) output<<*it;
  return output;
...
template <class T>
void State<T>::addDaughter(const State<T>& daughter, Ratio* ratio){
...
template <class T>
void State<T>::addDaughter(const State<T>& daughter){
